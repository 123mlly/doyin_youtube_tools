...
**中文默认文档**：[README.zh-CN.md](./README.zh-CN.md)（以下为英文说明）
...
**Full install (everything in one file):**

```bash
pip install -r requirements.txt
```

`requirements.txt` lists core + optional stacks (browser / GUI / YouTube / REST / transcribe / dev).  
If you only need a **minimal CLI** install, use editable mode and pick extras:

```bash
pip install -e .
# examples:
# pip install -e ".[browser,gui,youtube]"
```

**Playwright browsers** (required once for `cookie_fetcher` / browser fallback):
...
| Command | Description |
|---------|-------------|
| `douyin-dl` | Recommended CLI (see `pyproject.toml` `[project.scripts]`) |
| `python run.py` | Same CLI via project bootstrap |
| `douyin-dl-gui` | Desktop GUI (needs `PySide6`; included in full `requirements.txt`) |

GUI with YouTube panels also needs Google API libraries (`pip install -e ".[youtube]"` if you used a minimal install).
...
### 6) Desktop GUI (optional)

If `PySide6` is installed (`requirements.txt` or `pip install -e ".[gui]"`):
...
The GUI drives the same core as the CLI: multi-link download, cookie status + paste-to-save, YouTube auth/upload against `download_manifest.jsonl`, common settings (database, proxy, quiet logs), and a live log view. On macOS / Windows it can open a terminal to run `python -m tools.cookie_fetcher` (see Cookie tab).

### 7) Packaged GUI (macOS / Windows, PyInstaller)

Build a **folder-style** app (includes Qt plugins) from the repo root:

- **macOS / Linux shell:** `bash packaging/build_gui_release.sh`
- **Windows PowerShell:** `packaging\build_gui_release.ps1`
...
- **macOS:** `dist/DouyinDownloaderGui.app` (and a `dist/DouyinDownloaderGui/` support tree used by the bundle).
- **Windows:** `dist/DouyinDownloaderGui/` with `DouyinDownloaderGui.exe` inside — zip that folder for distribution.

Ship `config.yml` / cookies next to the app or tell users to copy `config.example.yml` from the bundle root. **Code signing / notarization (macOS)** and **Inno Setup / MSIX (Windows)** are not generated here; add them if you need installer UX beyond a zip. **Step-by-step:** [packaging/INSTALLERS.md](./packaging/INSTALLERS.md).

**Frozen app note:** “Open terminal for `cookie_fetcher`” uses a **Python on `PATH`** (`python3` / `python`) and expects to be run from a checkout that still contains the `tools/` package (or use **Copy command** and run inside your clone).
...
```bash
douyin-dl --hot-board 30 -p ./Downloaded
...
```bash
douyin-dl --search "猫咪" --search-max 100 -p ./Downloaded
...
```bash
pip install fastapi uvicorn       # one-time optional dep (or use full requirements.txt)
...
Uploads **primary MP4** entries referenced in `Downloaded/download_manifest.jsonl` (same base path as `path` in config). Duplicate `aweme_id` rows are skipped when `database: true` and history exists.
...
3. **Config**: set `youtube_upload.client_secret_path` / `token_path` (see `config.example.yml` block `youtube_upload:`).
4. **Authorize once**:

   ```bash
   douyin-dl --youtube-auth
   ```

5. **Upload latest manifest records**:

   ```bash
   douyin-dl --youtube-upload-latest 5
   douyin-dl --youtube-upload-latest 3 --youtube-dry-run   # plan only, no API upload
   ```

Use `youtube_upload.auto_after_download: false` (default) if you **do not** want uploads during normal download runs; use the CLI flags or GUI YouTube tab for on-demand uploads.
...
Recommended (from the repo root, dev deps are in `requirements.txt` or `pip install -e ".[dev]"`):
...
| `youtube_upload.*` | Optional YouTube upload: OAuth paths, privacy, templates, `auto_after_download`, `dry_run`, `max_items_per_run` |
