...
class BaseDownloader(ABC):
    ...
    async def _upload_manifest_record_to_youtube(
        self, manifest_record: Dict[str, Any]
    ) -> None:
        if not self.youtube_uploader or not getattr(
            self.youtube_uploader, "auto_after_download", False
        ):
            return
        try:
            await self.youtube_uploader.upload_manifest_record(
                self.file_manager.base_path, manifest_record
            )
        except Exception as exc:
            logger.warning("YouTube upload hook failed: %s", exc)
    ...
    async def _download_aweme_assets(
        ...
        await self._upload_manifest_record_to_youtube(manifest_record)
