...
def test_youtube_upload_defaults_and_yaml_merge(tmp_path):
    config_file = tmp_path / "config.yml"
    config_file.write_text(
        """
link:
...
youtube_upload:
  enabled: true
  dry_run: true
  title_template: "Douyin {aweme_id}"
"""
    )

    loader = ConfigLoader(str(config_file))
    youtube_upload = loader.get("youtube_upload", {})

    assert youtube_upload["enabled"] is True
    assert youtube_upload["dry_run"] is True
    assert youtube_upload["title_template"] == "Douyin {aweme_id}"
    assert youtube_upload["auto_after_download"] is False
    ...
    assert youtube_upload.get("upload_chunk_size_bytes") == 8 * 1024 * 1024
