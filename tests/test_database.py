...
@pytest.mark.asyncio
async def test_database_youtube_upload_history_upsert(tmp_path):
    db_path = tmp_path / "test.db"
    database = Database(str(db_path))
    await database.initialize()

    assert await database.is_youtube_uploaded("123") is False

    await database.upsert_youtube_upload_history(
        {
            "aweme_id": "123",
            "video_id": None,
            "status": "failure",
    ...
    assert await database.is_youtube_uploaded("123") is True
