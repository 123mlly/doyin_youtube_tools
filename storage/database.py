...
class Database:
    ...
    async def initialize(self):
        ...
        await db.execute('CREATE INDEX IF NOT EXISTS idx_youtube_upload_aweme_id ON youtube_upload_history(aweme_id)')
        await db.execute('CREATE INDEX IF NOT EXISTS idx_youtube_upload_status ON youtube_upload_history(status)')
    ...
    async def is_youtube_uploaded(self, aweme_id: str) -> bool:
        db = await self._get_conn()
        cursor = await db.execute(
            'SELECT id FROM youtube_upload_history WHERE aweme_id = ? AND status = ?',
            (aweme_id, 'success'),
        )
        result = await cursor.fetchone()
        return result is not None

    async def upsert_youtube_upload_history(self, upload_data: Dict[str, Any]):
        now_ts = int(datetime.now().timestamp())
        db = await self._get_conn()
        await db.execute('''
            INSERT INTO youtube_upload_history (
                aweme_id,
                video_id,
                status,
                error_message,
