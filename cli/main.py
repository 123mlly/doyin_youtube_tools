...
async def download_url(
    ...
    async with DouyinAPIClient(
        ...
        if result and database:
            safe_config = _safe_history_config(config.config)
...
async def main_async(args):
    ...
    _apply_youtube_cli_overrides(args, config)

    # 独立子命令：热榜 / 搜索 / 服务 / YouTube 授权与补传
...
def _apply_youtube_cli_overrides(args, config: ConfigLoader) -> None:
    overrides = {}
    if getattr(args, "youtube_dry_run", False):
    ...
    if overrides:
        config.update(youtube_upload=overrides)


async def _run_youtube_auth_subcommand(config: ConfigLoader) -> None:
    try:
        display.print_info("Opening YouTube authorization flow...")
        token = run_youtube_auth(youtube_settings(config))
    except Exception as exc:
        display.print_error(f"YouTube authorization failed: {exc}")
        return
    token_path = youtube_settings(config).get("token_path", "config/youtube_token.json")
...
async def _run_youtube_upload_latest_subcommand(args, config: ConfigLoader) -> None:
    ...
    try:
        ...
        results = await publish_latest_from_manifest(
            ...
            base_path,
            int(args.youtube_upload_latest or 0),
            database=database,
        )
    except Exception as exc:
        display.print_error(f"YouTube upload latest failed: {exc}")
        return
    finally:
        if database is not None:
            await database.close()
...
def _safe_history_config(raw_config: dict) -> dict:
    ...
    youtube_upload = safe_config.get("youtube_upload")
    if isinstance(youtube_upload, dict):
        safe_youtube_upload = {
            k: v
            for k, v in youtube_upload.items()
            if k
            not in {
                "client_secret",
                "client_secret_path",
        ...
        safe_config["youtube_upload"] = safe_youtube_upload
    return safe_config
...
def main():
    ...
    parser.add_argument(
        '--youtube-auth',
        action='store_true',
        help='执行 YouTube OAuth 授权并保存本地 token',
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--youtube-dry-run',
        action='store_true',
        help='只展示 YouTube 上传计划，不调用 YouTube API',
    )
    try:
